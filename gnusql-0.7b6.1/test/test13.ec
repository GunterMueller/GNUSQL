/*
 * $Id: test13.ec,v 1.2 1998/09/29 21:26:56 kimelman Exp $
 *
  ...
  main(void)
{
  ...
  DECLARE CURS1 CURSOR FOR
    ( 
      SELECT TABNAME, OWNER, UNTABID, NCOLS
      FROM DEFINITION_SCHEMA.SYSTABLES
      WHERE NCOLS - 9 > 
        (
          SELECT COLNO --UNTABID
          FROM DEFINITION_SCHEMA.SYSCOLUMNS
          WHERE UNTABID = 2 AND COLNAME = 'UNTABID'
          --COLNO = 5 --'COLNOTO1' --'ABCD'
        )
    )
;
  ...
  while(1)
    {
      $ fetch CURS1 into :tabname,:owner,:untabid, :ncols;
