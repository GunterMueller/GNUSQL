...
/* $Id: helpfu.c,v 1.246 1998/05/20 05:49:51 kml Exp $ */
...
extern char pl1[RPAGE], pl2[RPAGE];
extern char *p[2];
...
get_rec (char *pnt, struct ADBL adj, i4_t fd)
{
  ...
  if (n < RTBLK)
    {			/* the top-block places in two pages*/
      char buff[size2b];
      n1 = RTBLK - n;
      bcopy (p[k] + RTPAGE, buff + n1, n);
      k = get_page (k, pushpn[k] - 1, fd);
      end_of_record = p[k] + RPAGE - n1;
    ...
    {			/* the top-block places in (N)-page */
      end_of_record = p[k] + n + RTPAGE - RTBLK;
      razm = t2bunpack (end_of_record);
      n -= RTBLK;
      if (n == 0)
        {
          k = get_page (k, pushpn[k] - 1, fd);
          end_of_record = p[k] + RPAGE;
          n = RPAGE - RTPAGE;
        }
    }
  t2bpack (razm, pnt);
  ...
  if (n < razm)
    {			/* block-record places in two pages */
      n1 = razm - n;
      bcopy (p[k] + RTPAGE, pnt + n1, n);
      k = get_page (k, pushpn[k] - 1, fd);
      bcopy (p[k] + RPAGE - n1, pnt, n1);
    }
...
do_cont (void)
{
  ...
  *(p[ilast] + size4b + size2b) = SIGN_CONT;
  out_push (ilast, 0);
